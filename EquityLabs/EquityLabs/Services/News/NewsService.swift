...
class NewsService: ObservableObject {
    ...
    func fetchNews(for symbol: String, count: Int = 10, forceRefresh: Bool = false) async throws -> [NewsArticle] {
        isLoading = true
        ...
        do {
            ...
            cacheNews(response.articles, for: symbol)

            AppLogger.portfolio.info("Fetched \(response.articles.count) news articles for \(symbol)")
    ...
    func summarizeArticle(url: String) async throws -> String {
        isLoading = true
        defer { isLoading = false }

        do {
            let request = NewsSummaryRequest(url: url)
            let response: NewsSummaryResponse = try await apiClient.upload(
                .summarizeNews,
                body: request
            )

            AppLogger.portfolio.info("Generated summary for article: \(url)")
    ...
    private func cacheNews(_ articles: [NewsArticle], for symbol: String) {
        let cached = CachedNews(articles: articles, symbol: symbol.uppercased())
    ...
    func fetchNewsForSymbols(_ symbols: [String], count: Int = 5) async throws -> [String: [NewsArticle]] {
        ...
        try await withThrowingTaskGroup(of: (String, [NewsArticle]).self) { group in
            for symbol in symbols {
                group.addTask {
                    let articles = try await self.fetchNews(for: symbol, count: count)
